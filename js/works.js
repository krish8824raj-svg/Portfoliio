    ...
    if (pageTitleEl) {
      pageTitleEl.textContent = projectsHeading;
    }

    var PROJECTS = projectsSource.map(function(proj) {
      ...
      return {
        id: proj.id || '',
        title: proj.title || '',
        desc: isEn ? (proj.descEn || proj.descFr || '') : (proj.descFr || proj.descEn || ''),
        category: isEn ? (proj.categoryEn || proj.categoryFr || '') : (proj.categoryFr || proj.categoryEn || ''),
        year: proj.year || '',
        tags: tags,
        cover: fixPath(proj.cover)
      };
    });
