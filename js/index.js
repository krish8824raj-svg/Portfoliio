...
function setupProjectsSection() {
  ...
  // Parse dynamic data from our central config file
  var activeData = window.PORTFOLIO_ACTIVE_DATA || window.PORTFOLIO_DATA || { projects: [] };
  var projectsSource = activeData.projects || [];
  
  const PROJECTS = {};
  projectsSource.forEach(function(proj) {
    var tags = proj.tags;
    ...
    if (typeof images === 'string') {
      try {
        ...
        images = images.split('\n').map(function(img) { return img.trim(); }).filter(function(img) { return img.length > 0; });
      }
    }
    ...
    PROJECTS[proj.id] = {
      desc: proj.desc || proj.descEn || proj.descFr || '',
      category: proj.category || proj.categoryEn || proj.categoryFr || '',
      year: proj.year || '',
      tags: tags,
      images: images,
  ...
  const detailLinkWrap = document.getElementById('detail-link-wrap');
  ...
  function renderSelectedMedia(src) {
    if (!src) {
    ...
    if (isVideoPath(src)) {
      detailSelected.innerHTML = '<video src="' + src + '" autoplay muted loop playsinline controls style="max-width:100%; max-height:100%; object-fit:contain; background:#000; border-radius:4px; outline:none;"></video>';
    } else {
      detailSelected.innerHTML = '<img src="' + src + '" alt="" decoding="async" style="max-width:100%; max-height:100%; object-fit:contain; border-radius:4px;">';
    }
    _cachedSelImg = detailSelected.querySelector('img, video');
  }
  ...
  function updateActiveThumb() {
    ...
    if (closestIdx !== _activeThumbIdx) {
      ...
      var activeSrc = _thumbImgs[closestIdx].getAttribute('data-src');
      renderSelectedMedia(activeSrc);
    }
  ...
  function openProject(id, clickedItem) {
    ...
    if (proj.liveUrl && proj.liveUrl.trim() !== '') {
      detailLinkWrap.style.display = 'block';
      detailLiveLink.href = proj.liveUrl;
      // Re-initialize hover effect if necessary or just rely on existing setup
    } else {
      detailLinkWrap.style.display = 'none';
    }
    
    var allImages = [clickedItem.dataset.img].concat(proj.images).filter(function(s) { return !!s; });
    
    detailThumbsInner.innerHTML = allImages.map(function (src) {
      if (isVideoPath(src)) {
        return '<div class="thumb-media" data-src="' + src + '" style="position:relative; width:100%; height:80px; border-radius:4px; overflow:hidden; border:1px solid var(--border-color); background:#000; cursor:pointer; margin-bottom:10px;"><video src="' + src + '" muted playsinline style="width:100%; height:100%; object-fit:cover; pointer-events:none;"></video><span style="position:absolute; bottom:4px; right:4px; font-size:8px; background:rgba(0,0,0,0.7); color:#fff; padding:1px 3px; border-radius:2px;">VIDEO</span></div>';
      } else {
        return '<div class="thumb-media" data-src="' + src + '" style="width:100%; height:80px; border-radius:4px; overflow:hidden; border:1px solid var(--border-color); cursor:pointer; margin-bottom:10px;"><img src="' + src + '" alt="" decoding="async" style="width:100%; height:100%; object-fit:cover;"></div>';
      }
    }).join('');

    renderSelectedMedia(allImages[0]);

    _thumbImgs = [].slice.call(detailThumbsInner.querySelectorAll('.thumb-media'));
    _thumbImgs.forEach(function (thumb, i) {
      thumb.addEventListener('click', function () {
        if (_activeThumbIdx >= 0 && _thumbImgs[_activeThumbIdx]) _thumbImgs[_activeThumbIdx].classList.remove('active');
        thumb.classList.add('active');
        _activeThumbIdx = i;
        renderSelectedMedia(thumb.getAttribute('data-src'));
      });
    });

    _activeThumbIdx = 0;
    if (_thumbImgs[0]) _thumbImgs[0].classList.add('active');
    _galleryY = 0;
    ...
    if (proj.liveUrl && proj.liveUrl.trim() !== '') {
      tl.fromTo(detailLinkWrap, { opacity: 0 }, { opacity: 1, duration: 0.5, ease: 'power2.out' }, 1.3);
    }
...
function resetCrossPageTransitionState() {
  ...
  if (workFlyText) {
    var activeData = window.PORTFOLIO_ACTIVE_DATA || window.PORTFOLIO_DATA || {};
    var projectsHeading = activeData.projectsHeading || 'Work';
    gsap.killTweensOf(workFlyText);
    gsap.set(workFlyText, { opacity: 0, x: 0, y: 0, xPercent: 0, yPercent: 0 });
    workFlyText.textContent = projectsHeading;
  }
